...
- `add_dns_entries` (Boolean) When enabled, the DHCP server creates local DNS entries for its leases. Available since RouterOS 7.23.
- `add_dns_entries_suffix` (String) DNS name suffix appended to the entries added for leases; used only when add-dns-entries is enabled. Available since RouterOS 7.23.
...
- `disabled` (Boolean)
- `dynamic_lease_identifiers` (String) Dynamic lease identifier. RouterOS requires at least one identifier on modify, so this field is preserved (DiffSuppressed) when not set in user config — clearing it would cause apply to error with "at least one dynamic lease identifier should be specified".
